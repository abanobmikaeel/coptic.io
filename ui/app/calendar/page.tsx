...
import { GREGORIAN_MONTHS, COPTIC_MONTHS, WEEKDAYS, getFastColors } from '@/constants';
...
export default function CalendarPage() {
	...
	const loading = calendarData === null ||
	...
	const selectedDayData = selectedDay > 0 && selectedDay <= days.length ? days[selectedDay - 1] : null;
		...
		<main className="min-h-screen relative">
			...
			<section className="relative pt-20 pb-8 px-6">
				<div className="max-w-4xl mx-auto text-center">
					...
					<p className="text-gray-600 dark:text-gray-400">View fasting periods throughout the year</p>
				</div>
			</section>
				...
				<div className="max-w-4xl mx-auto">
					...
					<div className="flex items-center justify-between mb-6">
						...
						<div className="flex items-center gap-3">
							...
							<MonthYearSelector
								...
								onMonthChange={(m) => { setMonth(m); setSelectedDay(1); }}
								onYearChange={(y) => { setYear(y); setSelectedDay(1); }}
					...
					<div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 mb-4 shadow-sm dark:shadow-none">
						<div className="grid grid-cols-7 mb-2">
							{WEEKDAYS.map(day => (
								<div key={day} className="text-center text-sm font-medium text-gray-600 dark:text-gray-400 py-3">{day}</div>
							))}
						</div>
									...
									return (
										...
										>
											<span className={`text-lg font-semibold ${
												...
												colors ? `${colors.text} ${colors.darkText}` : 'text-gray-700 dark:text-gray-300'
											}`}>
												{(!features.copticCalendarMode || mode === 'gregorian') ? dayNum : dayData.copticDate.day}
											</span>
											{colors && (
												<span className={`absolute bottom-1.5 text-xs font-bold opacity-80 ${colors.text} ${colors.darkText}`}>
													{colors.icon}
												</span>
											)}
										</button>
							...
							{Array.from(visibleFasts).map(fast => {
								...
								return (
									<div key={fast} className="flex items-center gap-2.5">
										<div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold ${colors.bg} ${colors.darkBg} ${colors.text} ${colors.darkText}`}>
											{colors.icon}
										</div>
...
const MonthYearSelector = memo(function MonthYearSelector({
	...
	const selectClass = "text-xl font-bold text-gray-900 dark:text-white bg-transparent border-none cursor-pointer hover:text-amber-600 dark:hover:text-amber-500 transition-colors focus:outline-none focus:ring-0 appearance-none";

	if (!features.copticCalendarMode || mode === 'gregorian') {
		return (
			<>
				<select value={month - 1} onChange={(e) => onMonthChange(Number(e.target.value) + 1)} aria-label="Select month" className={`${selectClass} pr-1`}>
					{GREGORIAN_MONTHS.map((m, i) => (
						<option key={m} value={i} className="bg-white dark:bg-gray-900">{m}</option>
					))}
				</select>
				<select value={year} onChange={(e) => onYearChange(Number(e.target.value))} aria-label="Select year" className={selectClass}>
...
const SelectedDayDetails = memo(function SelectedDayDetails({
	...
	dayData: { gregorianDate: string; copticDate: { dateString: string }; fasting: { isFasting: boolean; fastType: string | null; description: string | null } };
	onClose: () => void;
}) {
	const colors = getFastColors(dayData.fasting.isFasting ? dayData.fasting.description : null);
		...
		<div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 mb-6 shadow-sm dark:shadow-none animate-in fade-in slide-in-from-top-2 duration-200">
			...
			<div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-800">
				{colors && dayData.fasting.description ? (
					<>
						<div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full ${colors.bg} ${colors.darkBg}`}>
